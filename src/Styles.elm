...
import Css exposing (..)
import Css.Global exposing (children, class, everything, global, selector, typeSelector)
...
style (Palette palette) =
    let
        (PaletteProperties primaryStyle) =
            palette.primary

        (PaletteProperties secondaryStyle) =
            palette.secondary
    in
    global
        [ typeSelector "body"
        ...
        , typeSelector "ul"
            [ maxHeight <| calc (pct 100) minus (toPx <| listHeaderHeight + listFooterHeight)
            , listStyle none
            , margin zero
            , padding zero
            , overflowY auto
        ...
        , class "board--column"
            [ height <| calc (pct 100) minus (toPx <| gap + scrollBarThickNess)
